...
/*   Copyright (C) 2011-2017, tekHedd, Owen Williams, Tobias Rafreider   */
...
// Tune the spindown timer to match the amount of tension on your wheels.
// 100 is fine for free-spinning wheels.
//
// Fast spindown timer uses more CPU, but does a better job of matching forward
// spins and scratching. Try scratching with a little resistance on the wheel
// and spindown set at 30.
//
// It's probably pointless to set this much less than 20, and will simply result
// in a ~20ms timeout.
// 
VestaxVCI400.spindownPollMs = 30; // 100 is easy on CPU, 30-50 is better with damped spinners

//
// Set higher for less pitch bend sensitivity, smaller for more. if you like
// a little damping on the wheels, you probably want a bit more sensitivity
// as well.
//
// 40 - default - for free-spinning wheels
// 20 - a bit touchy
// 10 - sensitive, good for highly damped wheels
// Don't set to 0!
VestaxVCI400.jogDivider = 30;
...
VestaxVCI400.init = function (id) {
   ...
   // Create wheel touch release timer
   VestaxVCI400.backgroundTickTimer = engine.beginTimer(
        VestaxVCI400.spindownPollMs, "VestaxVCI400.tick()", false);
};
...
/**
 * Background tick for wheel touch release processing
 */
VestaxVCI400.tick = function() {
   VestaxVCI400.Decks.A.tick();
   VestaxVCI400.Decks.B.tick();
   VestaxVCI400.Decks.C.tick();
   VestaxVCI400.Decks.D.tick();
};
...
VestaxVCI400.Deck = function (deckNumber, group, active) {
   ...
   this.isSpinning = false; // true during spinback after a wheel touch
}
/*
...
VestaxVCI400.Deck.prototype.onWheelTouch = function(value) {
    if (value === VestaxVCI400.ButtonState.pressed) {
        // Only scratchEnable() if it is not already enabled, to avoid glitching.
        if (! this.isSpinning) {
            // Mixed reports: is wheel 4096 or 2400 ticks per revolution?
            engine.scratchEnable(this.deckNumber, 2400, 33.3333, 0.125, 0.125/32, true);
        }
        
        this.isSpinning = false; // We have grabbed it; don't exit scratch mode
    }
    else {
 ...
 */
VestaxVCI400.Deck.prototype.tick = function() {
    if (this.isSpinning) {
        this.finishWheelTouch();
    }
};
...
VestaxVCI400.Deck.prototype.finishWheelTouch = function() {
    // We always want a smooth handoff from wheelspin to the current state.
    // When !PLAYING, wait for a full stop so we can spindown. That's cool.
    // When PLAYING
    //   - If spinning backwards, wait for a complete stop
    //   - If spinning forwards, wait for spin speed to be <= normal playback speed.

    var scratchRate = engine.getValue(this.group, "scratch2");
    var play = engine.getValue(this.group, "play");
    var stopNow = false;
    var smooth = true;
    
    // Playback wobbles when you disengage scratch when spinning forward
    // faster than normal playback speed.
    // So, special case, if the scratch rate is ~ exactly the play speed,
    // don't smooth the cutover.
    if (play !== 0) {           // playing?
        if (scratchRate > 1.15) {       // spinning forwards
            // do nothing
        }
        else if (scratchRate > 0.9) {  // Very close to playback speed
            stopNow = true;
            smooth = false;
        }
        else if (scratchRate > -0.15) { // backwards
            stopNow = true;
        }
        // else spinning backwards
    } else {                    // ~stopped
        if (scratchRate < 0.05 && scratchRate > -0.05) {
            stopNow = true;
        }
    }

    if (stopNow) {
        engine.scratchDisable(this.deckNumber, smooth);
        this.isSpinning = false;
    }
    else {
        this.isSpinning = true; // check at next tick()
    }
};
...
VestaxVCI400.Deck.prototype.onWheelMove = function(value) {
    ...
    if (engine.getValue(this.group, "scratch2_enable")){
        // engine.setValue(this.group, "jog", jogValue / 40); // Tune to match the default scratchEnable() parameters
        engine.scratchTick(this.deckNumber, jogValue);
    }
    else {
        if (engine.getValue(this.group, "play")) {
            engine.setValue(this.group, "jog", jogValue / VestaxVCI400.jogDivider); // Tune to taste
        } else {
            // For cueing, assuming "normal" is 256 ticks per rev and this is 2400...
            engine.setValue(this.group, "jog", jogValue / 4.6);                     
        }
    }
};
